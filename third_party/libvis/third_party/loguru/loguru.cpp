...
// ------------------------------------------------------------------------------
  ...
  #ifdef __EMSCRIPTEN__
  static void log_message_impl(int stack_trace_skip, int messageInt, int with_indentation, int abort_if_fatal)
  {
    Message& message = *reinterpret_cast<Message*>(messageInt);
  #else
  static void log_message_impl(int stack_trace_skip, Message& message, bool with_indentation, bool abort_if_fatal)
  {
  #endif
	...
	{
		#ifdef __EMSCRIPTEN__
		  // Avoid deadlocks if using emscripten with proxying to the main thread
		  // (locking loguru's mutex first, and then calling [f]fprintf(), which internally proxies to the main thread, may deadlock
		  //  if the main thread itself may call the logging function, since it may wait for locking loguru's mutex, which another
		  //  thread is holding that waits for proxying to the main thread)
		  emscripten_sync_run_in_main_runtime_thread(EM_FUNC_SIG_VIIII, &log_message_impl, stack_trace_skip, reinterpret_cast<int>(&message), with_indentation, abort_if_fatal);
		#else
		  log_message_impl(stack_trace_skip, message, with_indentation, abort_if_fatal);
		#endif
	}
