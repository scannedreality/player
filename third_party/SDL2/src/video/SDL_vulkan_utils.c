    ...
    Uint32 *extensionCount)
{
    ...
    // Calling vkEnumerateInstanceExtensionProperties() with count == 0, but a non-null retval
    // is an unusual case that might be likely to hit driver bugs, so let's avoid it.
    if (count == 0) {
    ...
    // VK_INCOMPLETE should not be returned unless there is a driver bug
    // (or in the extremely unlikely case that there are underlying changes since the first call).
    // However, a user did send a log file saying that this happened.
    // So, we try to continue here in that case.
