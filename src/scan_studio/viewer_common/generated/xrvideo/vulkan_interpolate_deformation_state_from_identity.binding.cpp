...
#include "vulkan_interpolate_deformation_state_from_identity.binding.hpp"

#include <libvis/vulkan/device.h>
#include <libvis/vulkan/shader.h>

#include "vulkan_interpolate_deformation_state_from_identity.comp.h"

using namespace vis;

namespace vulkan_interpolate_deformation_state_from_identity {
...
bool CreateDescriptorSet0Layout(VulkanDescriptorSetLayout* layout, const VulkanDevice& device, const DescriptorSet0Config* config) {
  layout->AddBinding(0, (config && config->StateDynamic) ? VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC : VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_COMPUTE_BIT);
  layout->AddBinding(1, (config && config->OutputDynamic) ? VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC : VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_COMPUTE_BIT);
  
  return layout->Initialize(device);
}

void DescriptorSet0::Update(
      const VkDescriptorBufferInfo* State,
  ...
  writeDescriptorSet[0].pBufferInfo = State;
...
bool Program::Initialize(u32 workgroupSizeX, const VkPipelineCache& pipelineCache, const VulkanDevice& device, int descriptorSet0Count, const DescriptorSet0Config* descriptorSetConfig0) {
  descriptorPool.AddPoolSize((descriptorSetConfig0 && descriptorSetConfig0->StateDynamic) ? VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC : VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, descriptorSet0Count * 1);
  ...
  if (!shader.Initialize(vulkan_interpolate_deformation_state_from_identity_comp, VK_SHADER_STAGE_COMPUTE_BIT, device, "main")) {
    return false;
  }
